...
namespace Soulstone.Duality.Editor.Blue.Utility
{
    public static class TypeExtMethods
    {
        ...
        public static string GetFriendlyName(this Type type, int recursionLimit = 100)
        {
            ...
            if (recursionLimit < 1)
            {
                Logs.Editor.WriteWarning(nameof(GetFriendlyName) + " from the Blue UI plugin reached its recursion limit.");
                return "";
            }
            ...
            if (Aliases.ContainsKey(type))
                ...
                name = type.GetElementType().GetFriendlyName(--recursionLimit) + "[]";
            ...
            if (type.IsNested && !type.IsGenericParameter)
                name = type.DeclaringType.GetFriendlyName(--recursionLimit) + "+" + name;
